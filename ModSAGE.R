...
compute_statistic = function(data,other_feats,n_inds,trees,k_set,feature_trees,roots_at_bar_trees){
  ...
  for(i in 1:1){
    ...
    ret = rep(0,n_inds)
    for(j in 1:n_inds){
    #ret = foreach(j = 1:n_inds,.packages = "data.table",.combine = "c",.export = c("feat_i")) %dopar% {
...
ModSage = function(data,trees,feature){
...
ModSage_cpp = function(data,trees,feature,loss){
